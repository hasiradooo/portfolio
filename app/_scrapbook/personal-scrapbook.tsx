...
import { BIRTH_DATE, favoriteBands, favoriteGames, personalAge } from "@/lib/personal";
...
const extraGames = [
  {"name": "The Binding of Isaac: Rebirth", "image": "/scrapbook/games/binding-of-isaac.webp", "url": "https://store.steampowered.com/app/250900/", "note": "just one more basement run.", "rotation": "-3deg"},
    ...
    <div lang="en" className={`${styles.world} ${autumn ? styles.autumn : ""}`} data-motion={animated ? "on" : "off"} data-personal-page data-personal-palette={autumn ? "autumn" : "blue"}>
      ...
      <div className={styles.canvas}>
        ...
        <section id="games" className={styles.games} aria-labelledby="games-title">
          <div className={styles.musicHeading}>
            <div><span className={styles.smallLabel}>THE OTHER KIND OF PLAY BUTTON</span><h2 id="games-title">one more <em>game.</em></h2></div>
          ...
          <div className={styles.gameGrid}>
            {[...favoriteGames, ...extraGames].map((game, i) => (
              <a key={game.name} href={game.url} target="_blank" rel="noopener noreferrer" className={`${styles.polaroid} ${styles.gameCard}`} style={tilt(game.rotation)} aria-label={`${game.name}: official game page (opens in a new tab)`}>
                <span className={i % 2 === 0 ? styles.pin : styles.tape} aria-hidden="true" />
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={game.image} width="460" height="215" alt={`${game.name} cover artwork`} loading="lazy" decoding="async" />
                <span className={styles.gameTag}>FAVORITE / 0{i + 1}</span>
                <h3>{game.name}</h3><p>{game.note}</p>
                <span className={styles.gameCorner} aria-hidden="true"><ScrapbookIcon name="arrow" /></span>
              </a>
            ))}
          </div>
          <p className={styles.gameFootnote}>PC games, tabletop games. Just let me play. ♡</p>
        </section>
