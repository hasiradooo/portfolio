...
export default function ContactPage() {
	...
	const contactMethods = [
		{
			...
			description:
				dict?.contact?.otherWays?.email?.description ||
		...
		},
		{
			...
			description:
				dict?.contact?.otherWays?.discord?.description ||
				"Quick chats and questions",
		...
		},
		{
			...
			description:
				dict?.contact?.otherWays?.telegram?.description ||
		...
		},
		{
			...
			description:
				dict?.contact?.otherWays?.twitter?.description || "Follow my journey",
		...
		},
		{
			...
			value:
				dict?.contact?.otherWays?.bsky?.value || "@hasiradoo.rabbittale.co‬",
			description:
				dict?.contact?.otherWays?.bsky?.description || "Follow my journey",
	...
	return (
		<>
			...
			<section
				id="contact-header"
				className="mb-12 max-w-screen-xl mx-auto px-0"
			>
				<div className="border-l-4 border-foreground pl-4">
					...
					<p className="mt-4 text-gray-600 dark:text-gray-400">
						{dict?.contact?.header?.description ||
					...
					<Form {...form}>
						...
						>
									...
									<FormItem>
										...
										<FormControl>
											<Input
												type="text"
												placeholder={
													dict?.contact?.form?.fields?.namePlaceholder ||
													"Your name"
												}
									...
									<FormItem>
										...
										<FormControl>
											<Input
												type="email"
												placeholder={
													dict?.contact?.form?.fields?.emailPlaceholder ||
													"your@email.com"
												}
												disabled={isSubmitting}
												className="w-full px-4 py-2 text-sm border rounded-none focus:ring-2 focus:ring-foreground/20"
									...
									<FormItem>
										...
										<FormControl>
											<Textarea
												rows={6}
												placeholder={
													dict?.contact?.form?.fields?.messagePlaceholder ||
							...
							<p className="text-xs text-muted-foreground text-center">
								{dict?.contact?.form?.recaptchaNotice ||
				...
				<section>
					...
					<div className="mt-6">
						...
						<div className="bg-background px-6">
							<p className="text-sm text-gray-600 dark:text-gray-400">
								{dict?.contact?.availability?.description ||
