...
export const metadata = {
	title: "Dialogue study | Kiro’s notebook",
...
const sample: StoryParagraph[] = [
	...
	{
		runs: [
			{ text: "[[memory speaker=rin to=kiro]] " },
			{ text: "“You’re allowed to just exist here, idiot.”", italic: true },
		],
	},
	"His phone buzzed against his thigh. Susie was still across the table.",
	{
		runs: [
			{ text: "[[message speaker=susie to=kiro]] " },
			{ text: "want me to change the subject?", bold: true },
		],
	},
...
export default function DialoguePreview() {
	return (
		<>
			<aside className={dialogue.previewNote}>
				<span>READING EXPERIMENT / NOT A NEW CHAPTER</span>
				<p>
					A small scene to try clearer dialogue. Names sit beside spoken lines;
					shared brackets keep overlapping voices together.
				</p>
				<div className={dialogue.legend}>
					<span>names, not just colors</span>
					<span>remembered voices marked</span>
					<span>messages kept distinct</span>
				</div>
			</aside>
			<article className={styles.sheet}>
				<span className={styles.tape} aria-hidden="true" />
				<header className={styles.chapterHeader}>
					<p className={styles.label}>DIALOGUE STUDY / NON-CANON SAMPLE</p>
					<h1>Still</h1>
					<p className={styles.readingTime}>
						three people / one kitchen / a little too much thinking
					</p>
				</header>
				<div className={styles.prose}>
					<StoryParagraphs paragraphs={sample} />
				</div>
				<div className={styles.chapterEnd}>
					✶<span>end of the sample</span>✶
				</div>
			</article>
		</>
	);
