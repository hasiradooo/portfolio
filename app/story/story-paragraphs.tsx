...
import { parseParagraph, voiceName, type StoryRun, type StoryParagraph, type ParsedParagraph } from "./dialogue-schema";
...
function VoiceParagraph({ paragraph }: { paragraph: ParsedParagraph }) {
  ...
  return <div className={dialogue.voice} data-speaker={voice.speaker} data-kind={voice.kind}>
    <div className={dialogue.tag}>
      <span className={dialogue.name}>{voiceName(voice.speaker)}</span>
      {voice.kind !== "speech" && <span className={dialogue.kind}>{voice.kind === "memory" ? "remembered" : "message"}</span>}
      {voice.to?.length ? <span className={dialogue.recipient}>to {voice.to.map(voiceName).join(", ")}</span> : null}
    </div>
    <div className={dialogue.body}><p><InlineText runs={paragraph.runs} links={linksIn(paragraph.runs)} /></p></div>
  </div>;
}

function PlainParagraph({ runs, music }: { runs: StoryRun[]; music?: ChapterTrack }) {
  const text = textOf(runs);
  if (/^[\u2014\u2013*\s-]+$/.test(text)) return <hr className={styles.sceneBreak} />;
  const links = linksIn(runs);
  const youtube = links.find(link => youtubeIdFromUrl(link.href));
  if (youtube) {
    const track = music?.youtubeUrl ? music : { title: text.replace(/^Song:\s*/i, ""), artist: "", youtubeUrl: youtube.href };
    return <>{!/^Song:/i.test(text) && <p><InlineText runs={runs} links={links} /></p>}<ChapterCassette key={track.youtubeUrl} track={track} /></>;
  }
  ...
  if (/^\[note\b/i.test(text) && koFi) {
    ...
    return <aside className={styles.storyNote} aria-label="Author’s note">
  ...
  return <p><InlineText runs={runs} links={links} /></p>;
}

export default function StoryParagraphs({ paragraphs, music }: { paragraphs: StoryParagraph[]; music?: ChapterTrack }) {
  const parsed = paragraphs.map(parseParagraph);
  const output: ReactNode[] = [];
  for (let index = 0; index < parsed.length; index++) {
    const paragraph = parsed[index];
    const overlap = paragraph.voice?.kind === "speech" ? paragraph.voice.overlap : undefined;
    let end = index + 1;
    if (overlap) while (end < parsed.length && parsed[end].voice?.kind === "speech" && parsed[end].voice?.overlap === overlap) end++;
    if (overlap && end > index + 1) {
      output.push(<section key={index} className={dialogue.overlap} aria-label="Overlapping voices">
        <span className={dialogue.overlapHeading}>overlapping voices / at the same time</span>
        {parsed.slice(index, end).map((part, offset) => <VoiceParagraph key={offset} paragraph={part} />)}
      </section>);
      index = end - 1;
    } else output.push(paragraph.voice ? <VoiceParagraph key={index} paragraph={paragraph} /> : <PlainParagraph key={index} runs={paragraph.runs} music={music} />);
  }
  return <>{output}</>;
}
