...
import { chapters, characters, chapterHref, characterPortraits } from "./story-data";
...
export default function StoryHome() {
  return <>
    <section className={styles.cover}>
      ...
      <p className={styles.intro}>A cream-and-golden fox-cat in an olive sheep-covered sweater. A black cat with a dark hoodie and dry humor. A badger with a soft spot for old CDs and second-hand things.</p>
    ...
    <div className={styles.characterTeasers}>{characters.filter(c => c.group === "main").map((character, i) => <Link href={`/story/characters#${character.name.toLowerCase()}`} key={character.name} className={styles.characterTeaser} style={{ transform: `rotate(${i === 1 ? 2 : -2}deg)` }}><span className={styles.pin} aria-hidden="true" /><span className={`${styles.characterInitial} ${characterPortraits[character.id] ? styles.portraitFrame : ""}`} style={{ background: character.color }}>{characterPortraits[character.id] ? <img className={styles.portraitImage} src={characterPortraits[character.id].src} alt={characterPortraits[character.id].alt} width={1280} height={999} loading="lazy" decoding="async" /> : <>{character.name[0]}<small>{character.kind}</small></>}</span><h3>{character.name}</h3><p>{character.tagline}</p></Link>)}</div>
