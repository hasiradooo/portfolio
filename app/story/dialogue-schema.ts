...
export type Voice = { kind: 'speech' | 'memory' | 'message'; speaker: string; to?: string[]; overlap?: string };
...
export function parseParagraph(paragraph: StoryParagraph): ParsedParagraph {
  ...
  if (typeof paragraph !== 'string' && paragraph.voice) return { runs, voice: paragraph.voice };
  const text = runs.map(run => run.text).join('');
  const marker = text.match(/^\[\[(speech|memory|message) speaker=([a-z][a-z0-9-]*)(?: to=([a-z][a-z0-9-]*(?:,[a-z][a-z0-9-]*)*))?(?: overlap=([a-z][a-z0-9-]*))?\]\][ \t]*/);
  ...
  const body = runs.flatMap(run => {
    ...
    remaining -= skip;
    return skip === run.text.length ? [] : [{ ...run, text: run.text.slice(skip) }];
  });
  return { runs: body, voice: { kind: marker[1] as Voice['kind'], speaker: marker[2], to: marker[3]?.split(','), overlap: marker[4] } };
}
