...
export default function StoryShell({
	...
	useEffect(() => {
		if (menu.current)
			menu.current.open = window.matchMedia("(min-width: 901px)").matches;
		const selected = menu.current?.querySelector<HTMLElement>(
			'[aria-current="page"]',
		);
		const list = window.matchMedia("(min-width: 901px)").matches
			? menu.current
			: menu.current?.querySelector<HTMLElement>("[data-chapter-list]");
		if (selected && list)
			list.scrollTop +=
	...
	return (
		<div
			lang="en"
			className={`${personal.world} ${autumn ? personal.autumn : ""} ${styles.world}`}
			data-personal-page
			data-personal-palette={autumn ? "autumn" : "blue"}
		>
			<a href="#story-content" className={personal.skip}>
				Skip to story
			</a>
			<ScrapbookBar
				href="/"
				backLabel="back to my world"
				address="home / kiro / notebook.html"
			>
				<button
					type="button"
			...
			<div className={styles.layout}>
				<aside className={styles.sidebar} aria-label="Story navigation">
					<Link href="/story" className={styles.notebookTitle}>
						kiro’s
						<br />
						<span>notebook.</span>
					...
					<details ref={menu} className={styles.contents} open>
						<summary>
							...
							<span>
								{active
									? `${String(active.number).padStart(2, "0")} / ${chapters.length}`
									: "open"}
							</span>
						</summary>
						<nav className={styles.nav} aria-label="Chapters and characters">
							<div className={styles.navIntro}>
								<Link
									href="/story"
									aria-current={pathname === "/story" ? "page" : undefined}
								>
									the beginning / about
								</Link>
								<Link
									...
									aria-current={
										pathname === "/story/characters" ? "page" : undefined
									}
								>
									meet the characters <span>♡</span>
								</Link>
							</div>

							<p className={styles.navLabel}>
								THE CHAPTERS / {chapters.length} PAGES TO GET LOST IN
							</p>
								...
								{chapters.map((chapter) => (
									<li key={chapter.slug}>
										<Link
											...
											aria-current={
												active?.slug === chapter.slug ? "page" : undefined
											}
										>
					...
					<p className={styles.sidebarFooter}>
						written by hasira
						<br />
						<span>best read with something warm.</span>
					</p>
				</aside>
				<div
					id="story-content"
			...
			<footer className={styles.footer}>
				some stories take time. thanks for being here. ♡{" "}
				<Link href="/">
					back to the scrapbook <Icon name="arrow" />
				</Link>
			</footer>
		</div>
	);
