...
export const metadata = storyMetadata("Kiro’s story", "/personal/story", "A story by hasira. Meet Kiro, Rin and Susie, and follow their lives one chapter at a time.");
export default function StoryHome() {
  return <>
    <section className={styles.cover}>
      ...
      <div className={styles.actions}><Link className={styles.primary} href={chapterHref(chapters[0].slug)}>start reading <Icon name="arrow" /></Link><Link href="/personal/story/characters">meet the characters</Link></div>
    ...
    <div className={styles.characterTeasers}>{characters.map((character, i) => <Link href={`/personal/story/characters#${character.name.toLowerCase()}`} key={character.name} className={styles.characterTeaser} style={{ transform: `rotate(${i === 1 ? 2 : -2}deg)` }}><span className={styles.pin} aria-hidden="true" /><span className={styles.characterInitial} style={{ background: character.color }}>{character.name[0]}<small>{character.kind}</small></span><h3>{character.name}</h3><p>{character.tagline}</p></Link>)}</div>
